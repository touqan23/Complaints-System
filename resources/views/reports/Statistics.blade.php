...
<div class="container">
    <div class="header">
        ...
        <div class="date">Report Date: {{ now()->format('Y-m-d H:i') }}</div>
    </div>
    ...
    <div class="content">
        ...
        @if(isset($data['status_summary']) && $data['status_summary']->count() > 0)
            <div class="section">
                <h2 class="section-title">Complaints Distribution by Status</h2>
                <table>
                    <thead>
                    <tr>
                        <th>Status</th>
        ...
        @if(isset($data['department_distribution']) && $data['department_distribution']->count() > 0)
            <div class="section">
                <h2 class="section-title">Complaints Distribution by Department</h2>
                <table>
                    <thead>
                    <tr>
                        <th>Department</th>
                    ...
                    @foreach($data['department_distribution'] as $dept)
                        <tr>
                            <td>{{ $dept->department->name ?? 'Unspecified' }}</td>
        ...
        @if(isset($data['top_locations']) && $data['top_locations']->count() > 0)
            <div class="section">
                <h2 class="section-title">Top Complaint Locations</h2>
                <table>
                    <thead>
                    <tr>
                        <th>Rank</th>
                    ...
                    @foreach($data['top_locations'] as $index => $location)
                        <tr>
                            ...
                            <td>{{ $location->location ?? 'Unspecified' }}</td>
        ...
        @if(isset($data['top_employees']) && $data['top_employees']->count() > 0)
            <div class="section">
                <h2 class="section-title">Most Active Employees</h2>
                <table>
                    <thead>
                    <tr>
                        <th>Rank</th>
                ...
                <table>
                    ...
                    @foreach($data['employee_performance'] as $employee)
                        <tr>
                            ...
                            <td>{{ $employee->avg_processing_time ? number_format((int)$employee->avg_processing_time, 1) : 'N/A' }}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
    ...
    <div class="footer">
        <p>This report was automatically generated by the Complaint Management System</p>
        <p style="margin-top: 8px; font-size: 12px;">© {{ date('Y') }} All Rights Reserved</p>
    </div>
</div>
</body>
</html>
